...
@WebServlet("/api/deleteparameter")
public class DeleteParameter extends HttpServlet{
    protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (BufferedReader reader = request.getReader())
        {
            ...
            try (DBWrapper db = new DBWrapper(); Connection conn = db.getConnection())
            {
                ...
                DeleteParameter(conn, input, audit);
                
                conn.commit();
                
                result.put("success", true);
                result.put("message", "Parameter deleted successfully");
            }
        } 
        catch (Exception e)
        {
    ...
    private void DeleteParameter(Connection conn, JSONObject input, AuditUtil.AuditInfo audit) throws SQLException{
       ...
       int parameterId = input.getInt("stock_id");
       try (PreparedStatement stmt = conn.prepareStatement(strsql)) {
            ...
            stmt.setInt(5, parameterId);   // primary key for WHERE
            stmt.executeUpdate();
       }  
    }
